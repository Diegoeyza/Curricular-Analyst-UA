function filterAndCopyByType() {
  ...
  const uniqueIdCol2= headers.indexOf("UniqueID")+1;
  console.log(areaCol)
  var uniqueIdRange = rawDataSheet.getRange(2, uniqueIdCol2, lastRow, 1); // Column C, including row 1
  var areaTypeRange = rawDataSheet.getRange(2, areaCol, lastRow, 1); // Column AG, including row 1
  ...
  var newSpreadsheet = SpreadsheetApp.create(selectedType + " - "+ spreadsheetName);
...
function filterAndCopyAllByType() {
// Get the active spreadsheet and its name
var spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
var spreadsheetName = spreadsheet.getName();
...
// Loop through each unique type and process it
uniqueTypes.forEach(function (selectedType) {
  // Create a new spreadsheet for each type
  var newSpreadsheet = SpreadsheetApp.create(selectedType + " - " + spreadsheetName);
  ...
  // Notify the user about the new spreadsheet created
  SpreadsheetApp.getUi().alert("Filtering complete for type '" + selectedType + "'! New spreadsheet created: " + newSpreadsheet.getUrl());
});
}
