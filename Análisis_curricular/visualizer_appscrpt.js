...
function createDropdownSheet() {
  ...
  function updateLearningObjectivesDropdown(e) {
    ...
    if (sheet.getName() === "Course Dropdown" && range.getA1Notation() === "A2") {
      ...
      // Prepare the requirement names and IDs
      ...
      // If there are filtered requirements, apply them as a dropdown to D2
      if (requirementNames.length > 0) {
        var requirementRule = SpreadsheetApp.newDataValidation()
                          .requireValueInList(requirementNames)
                          .build();
        sheet.getRange(2, 4).setDataValidation(requirementRule);
      } else {
        // If no requirements are found, clear the validation for D2
    ...
    if (sheet.getName() === "Course Dropdown" && range.getA1Notation() === "D2") {
      ...
      // Set the first matching Objective ID in E5
      if (filteredObjectivesFromRequirement.length > 0) {
        var firstObjective = filteredObjectivesFromRequirement[0];
        // Find the corresponding Objective ID for the first objective
        var objectiveID = null;
        for (var j = 0; j < objectivesValues.length; j++) {
          if (objectivesValues[j][3] === firstObjective) { // Compare with objective text in column 4
            objectiveID = objectivesValues[j][1]; // Get the Objective ID from column 2
            break;
          }
        }
        sheet.getRange(5, 5).setValue(objectiveID); // Set the first matching Objective ID in E5
      } else {
        sheet.getRange(5, 5).clearContent(); // Clear if no objectives are found
      }
    }
  }
